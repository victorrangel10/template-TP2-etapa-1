...
void RealizaConsultaClinica(tClinica* clinica, FILE* bancoConsulta,
                            FILE* bancoLesoes, char* nomeAtendente,
                            char* cpfAtendente, char* crmAtendente) {
  ...
  clinica->consultas = realloc(clinica->consultas,
                               (clinica->nConsultas + 1) * sizeof(tConsulta*));
  ...
  printf(
      "CONSULTA NUMERO %d COM PACIENTE %s de idade %d com %d lesoes realizada "
      "e salva\n",
      clinica->nConsultas,
      ObtemNomeAgente(
          ObtemPacienteConsulta(clinica->consultas[clinica->nConsultas - 1])),
      ObtemIdadePaciente(
          ObtemPacienteConsulta(clinica->consultas[clinica->nConsultas - 1])),
      ObtemNLesoesConsulta(clinica->consultas[clinica->nConsultas - 1]));
...
tClinica* RecuperaClinicaBinario(FILE* bancoCLINICA, FILE* bancoMEDICOS,
                                 FILE* bancoPACIENTES, FILE* bancoLESOES,
                                 FILE* bancoSECRETARIOS, FILE* bancoCONSULTAS) {
  ...
  RecuperaPacientesClinica(c,
                           bancoPACIENTES);  // Recupera os pacientes do arquivo
...
void RecuperaSecretariosClinica(tClinica* clinica, FILE* banco) {
  ...
  printf("NUMERO DE SECRETARIOS DA CLINICA EH OFICIALMENTE %d",
...
void CadastraPacienteClinica(tClinica* clinica, FILE* pathBanco) {
  if (clinica) {
    ...
    clinica->pacientes = realloc(clinica->pacientes,
                                 (clinica->nPacientes + 1) * sizeof(tAgente*));
    clinica->pacientes[clinica->nPacientes] = paciente;
    clinica->nPacientes++;
    RegistraAgenteBancoDados(paciente, pathBanco);
    printf(
        "CADASTRO REALIZADO COM SUCESSO. PRESSIONE QUALQUER TECLA PARA VOLTAR "
        "PARA O MENU INICIAL\n");
    printf("###############################################################");
    while (getchar() != '\n')
      ;
...
void CadastraMedicoClinica(tClinica* clinica, FILE* Banco) {
  if (clinica) {
    ...
    clinica->medicos =
        realloc(clinica->medicos, (clinica->nMedicos + 1) * sizeof(tMedico*));
...
void CadastraSecretarioClinica(tClinica* clinica, FILE* banco) {
  if (clinica) {
    ...
    // Checagem de CPF
    if (TemCPFIgualClinica(clinica, ObtemCPFSecretario(secretario),
                           SECRETARIO)) {
    ...
    clinica->secretarios =
        realloc(clinica->secretarios,
                (clinica->nSecretarios + 1) * sizeof(tSecretario*));
...
int ObtemNMEdicosClinica(tClinica* c) { return c->nMedicos; }

int ChecaLoginClinica(tClinica* c, char* nome, char* cpf, char* crm) {
  // SE PRECISAR RETORNAR O SECRETARIO NO FUTURO, CRIE UMA VARIAVEL NA MAIN E
  // MANDE O ENDEREÇO PRA CA
  //  ASSIM CONSEGUE RETORNAR O VALOR LA PRA FORA E USAR, E AQUI RETORNA O
  //  SECRETARIO ENCONTRADO;
  ...
  //  printf("LOGIN DO SECRETARIO ANALISADO EH
...
void BuscaPacientesClinica(tClinica* clinica) {
  ...
  if (temNomeIgualPacientesClinica(clinica, nomeBuscado)) {
    ...
    while (getchar() != '\n')
      ;
    if (opt == 1) {
      insereDocumentoFila(clinica->filaImpressao, l, imprimeNaTelaLista,
                          imprimeEmArquivoLista, desalocaLista);
      printf(
          "LISTA ENVIADA PARA FILA DE IMPRESSAO. PRESSIONE QUALQUER TECLA PARA "
    ...
    printf(
        "NENHUM PACIENTE FOI ENCONTRADO. PRESSIONE QUALQUER TECLA PARA "
...
void GeraRelatorioGeral(tClinica* clinica) {
  int atendidos = 0, sumIdades = 0, masc = 0, fem = 0, outros = 0,
      sumTamLesoes = 0, totalLesoes = 0, cirurgias = 0, crioterapia = 0;
  ...
  for (size_t i = 0; i < clinica->nPacientes; i++) {
    ...
    for (size_t k = 0; k < clinica->nConsultas; k++) {
      if (strcmp(ObtemCPFAgente(clinica->pacientes[i]),
                 ObtemCPFAgente(
                     ObtemPacienteConsulta(clinica->consultas[k]))) == 0) {
        tmp++;
        if (tmp == 1) {
          atendidos++;
        }
  ...
  for (size_t i = 0; i < clinica->nConsultas; i++) {
    ...
    /* if (!TemConsultaComMesmoPaciente(clinica->consultas,
     clinica->consultas[i], clinica->nConsultas)) { if (tamVet == 0) { tamVet++;
             vetorCPF = malloc(tamVet * sizeof(char*));
             vetorCPF[tamVet - 1] =
     malloc(strlen(ObtemCPFAgente(ObtemPacienteConsulta(clinica->consultas[i])))
     + 1); strcpy(vetorCPF[tamVet - 1] ,
     ObtemCPFAgente(ObtemPacienteConsulta(clinica->consultas[i]))); atendidos++;
         }

         for (size_t k = 0; k < tamVet; k++) {
             if (strcmp(vetorCPF[k],
     ObtemCPFAgente(ObtemPacienteConsulta(clinica->consultas[i]))) == 0) {  //
     se eh igual a outro cpf continue; } else {  // senao tamVet++; vetorCPF =
     realloc(vetorCPF, tamVet * sizeof(char*)); vetorCPF[tamVet - 1] =
     malloc(strlen(ObtemCPFAgente(ObtemPacienteConsulta(clinica->consultas[i])))
     + 1); strcpy(vetorCPF[tamVet - 1],
     (ObtemPacienteConsulta(clinica->consultas[i])));
             }
         }
     }*/
  }

  tRelatorio* rel = criaRelatorio(sumIdades, clinica->nConsultas, masc, fem,
                                  outros, totalLesoes, sumTamLesoes, cirurgias,
                                  crioterapia, atendidos, clinica->nPacientes);
  ...
  while (getchar() != '\n')
    ;
  if (opt == 1) {
    insereDocumentoFila(clinica->filaImpressao, rel, imprimeNaTelaRelatorio,
                        imprimeEmArquivoRelatorio, desalocaRelatorio);
    printf(
        "DOCUMENTO ENVIADO PARA LISTA DE IMPRESSAO. PRESSIONE QUALQUER TECLA "
