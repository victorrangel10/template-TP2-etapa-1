...
void RealizaConsulta(tConsulta* consulta, tFila* fila, FILE* bancoConsulta,
                     FILE* bancoLesoes) {
...
char dataEntrada[15];
  scanf("%[^\n]%*c", dataEntrada);
  printf("DATA ENTRADAs  EH %s\n",dataEntrada);
  int dia,mes,ano;
  sscanf(dataEntrada, "%d/%d/%d", &dia, &mes, &ano);
  sprintf(consulta->data,"%d/%d/%d",dia,mes,ano);
  printf("DATA DA CONSULTA CONSERTADA EH %s",consulta->data);
...
tAgente* ObtemPacienteConsulta(tConsulta* c) { return c->paciente; }

int ObtemNLesoesConsulta(tConsulta* c) { return c->nlesoes; }
...
void CadastraLesao(tConsulta* consulta) {
  ...
  consulta->lesoes =
      realloc(consulta->lesoes, consulta->nlesoes * sizeof(tLesao*));
  ...
  printf(
      "LESAO REGISTRADA COM SUCESSO. PRESSIONE QUALQUER TECLA PARA VOLTAR AO "
      "MENU ANTERIOR\n");
  printf("############################################################\n");
  while (getchar() != '\n')
    ;
...
void CadastraReceita(tConsulta* consulta, tFila* fila) {
  ...
  printf("\n\n\n\n\n\n\n\\n\n\n DATA EH %s \n\n\n\n\n\n\n", consulta->data);
  tReceita* r = criaReceita(ObtemNomeAgente(consulta->paciente), type, nome,
                            tipomed, instruc, qtd, consulta->nomeAplicador,
                            consulta->CRM, consulta->data);

  insereDocumentoFila(fila, r, imprimeNaTelaReceita, imprimeEmArquivoReceita,
                      desalocaReceita);

  printf(
      "RECEITA ENVIADA PARA FILA DE IMPRESSAO. PRESSIONE QUALQUER TECLA PARA "
...
void GeraBiopsia(tConsulta* consulta, tFila* fila) {
  ...
  if (!TemLesaoCirurgicaConsulta(consulta)) {
    printf(
        "NAO E POSSIVEL SOLICITAR BIOPSIA SEM LESAO CIRURGICA. PRESSIONE "
        "QUALQUER TECLA PARA RETORNAR AO MENU ANTERIOR\n");
    while (getchar() != '\n')
      ;
  ...
  tBiopsia* b =
      criaBiopsia(ObtemNomeAgente(consulta->paciente), consulta->nlesoes,
                  ObtemCPFAgente(consulta->paciente), consulta->lesoes,
                  consulta->nomeAplicador, consulta->CRM, consulta->data);
  insereDocumentoFila(fila, b, imprimeNaTelaBiospsia, imprimeEmArquivoBiopsia,
                      desalocaBiopsia);
  printf(
      "SOLICITACAO DE BIOPSIA ENVIADA PARA FILA DE IMPRESSAO. PRESSIONE "
      "QUALQUER TECLA PARA RETORNAR AO MENU ANTERIOR\n");
  printf("############################################################\n");
  while (getchar() != '\n')
    ;
...
void GeraEncaminhamento(tConsulta* consulta, tFila* fila) {
  ...
  tEncaminhamento* e = CriaEncaminhamento(
      ObtemNomeAgente(consulta->paciente), ObtemCPFAgente(consulta->paciente),
      especialidade, motivo, consulta->nomeAplicador, consulta->CRM,
      consulta->data);
  insereDocumentoFila(fila, e, ImprimeTelaEncaminhamento,
                      ImprimeArquivoEncaminhamento, DesalocaEncaminhamento);

  printf(
      "ENCAMINHAMENTO ENVIADO PARA FILA DE IMPRESSAO. PRESSIONE QUALQUER TECLA "
...
void SalvaConsultaBin(FILE* bancoConsulta, FILE* bancoLesoes, tConsulta* c) {
  if (c) {
    ...
    /*fwrite(c->data, sizeof(char), strlen(c->data) + 1, bancoConsulta);
     ...
     fwrite(c->nomeAplicador, sizeof(char), strlen(c->nomeAplicador) + 1,
     bancoConsulta);
...
int TemConsultaComMesmoPaciente(tConsulta** consultas, tConsulta* c, int n) {
  ...
  for (size_t i = 0; i < n; i++) {
    printf("ITERACAO %d\n", i);
    printf("VENDO SE %s EH IGUAL A %s",
           ObtemNomeAgente(ObtemPacienteConsulta(consultas[i])),
           ObtemNomeAgente(ObtemPacienteConsulta(c)));

    if (ObtemCPFAgente(ObtemPacienteConsulta(consultas[i])) ==
            ObtemCPFAgente(ObtemPacienteConsulta(c)) &&
        !JaFoiAtendidoPaciente(ObtemPacienteConsulta(consultas[i]))) {
      printf("SIM EH IGUAL\n");
      tmp++;
