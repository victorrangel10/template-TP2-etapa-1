...
void RealizaConsulta(tConsulta* consulta, tFila* fila) {
    ...
    char dataCon[10];
    scanf("%s", dataCon);
    scanf("%d %d %d %d", &consulta->diabetes, &consulta->fumante, &consulta->alergia, &consulta->cancer);
    char tipoPele[5];
    scanf("%s", tipoPele);
...
void CadastraReceita(tConsulta* consulta, tFila* fila) {
    ...
    tReceita* r = criaReceita(ObtemNomeAgente(consulta->paciente), type, nome, tipomed, instruc, qtd, ObtemNomeMedico(consulta->medico), ObtemCRMMedico(consulta->medico), consulta->data);
